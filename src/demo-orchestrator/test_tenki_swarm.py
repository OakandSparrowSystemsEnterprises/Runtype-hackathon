import importlib.util
import pathlib
import unittest

MODULE_PATH = pathlib.Path(__file__).with_name("tenki_swarm.py")
SPEC = importlib.util.spec_from_file_location("tenki_swarm", MODULE_PATH)
tenki_swarm = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(tenki_swarm)

ARTIFACT_REF = "sha256:5386fdfcbc233f3b8da8ba274651d2174aa233e88dc4d35948f2189923f652e5"
DIGEST = ARTIFACT_REF.split(":", 1)[1]
EFFECT = "parent-shield.navigation"
PRINCIPAL = "agent-b"
LIVE_PAYLOAD = {
    "claim": {
        "artifact_ref": ARTIFACT_REF,
        "artifact_sha256": DIGEST,
        "authority": False,
        "capacity_bits_per_s": 320.0,
        "claim_hash": "b36d8b524ea6db6639a36d834070474b71828e83f8d61c20e2de88dd353740ad",
        "compute_plane": "tenki",
        "kind": "goi_l0_boundary_capacity",
        "omega_res": 256,
        "principal": PRINCIPAL,
        "requested_effect": EFFECT,
        "role": "derived_claim_only",
        "tau_ms": 25.0,
    },
    "ok": True,
}


def plan():
    return tenki_swarm.build_swarm_plan(ARTIFACT_REF, EFFECT, PRINCIPAL)


class TenkiSwarmContractTests(unittest.TestCase):
    def test_dynamic_derive_request_is_exact_verified_four_field_shape(self):
        request = tenki_swarm.build_derive_request(ARTIFACT_REF, EFFECT, PRINCIPAL)
        self.assertEqual(request, {
            "artifact_ref": ARTIFACT_REF,
            "artifact_sha256": DIGEST,
            "requested_effect": EFFECT,
            "principal": PRINCIPAL,
        })
        self.assertNotIn("authority", request)
        self.assertNotIn("omega_res", request)
        self.assertNotIn("tau_ms", request)

    def test_plan_never_grants_authority(self):
        current = plan()
        self.assertFalse(current["authority"])
        self.assertEqual(current["requested_effect"], EFFECT)
        self.assertEqual(current["principal"], PRINCIPAL)
        self.assertGreaterEqual(len(current["jobs"]), 3)
        self.assertTrue(all(job["authority"] is False for job in current["jobs"]))

    def test_missing_runtime_endpoint_is_pending_not_fake_live(self):
        original = tenki_swarm.TENKI_DERIVE_URL
        tenki_swarm.TENKI_DERIVE_URL = ""
        try:
            result = tenki_swarm.run_tenki_swarm(ARTIFACT_REF, EFFECT, PRINCIPAL)
        finally:
            tenki_swarm.TENKI_DERIVE_URL = original
        self.assertEqual(result["status"], "NEXT_PENDING")
        self.assertFalse(result["live"])
        self.assertFalse(result["authority"])

    def test_upstream_cannot_manufacture_authority(self):
        payload = {"ok": True, "claim": dict(LIVE_PAYLOAD["claim"], authority=True)}
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), payload, 1.0)

    def test_nested_claim_cannot_manufacture_token(self):
        claim = dict(LIVE_PAYLOAD["claim"])
        claim["details"] = {"token": "worker-issued-token"}
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), {"ok": True, "claim": claim}, 1.0)

    def test_claim_artifact_must_match_current_governed_artifact(self):
        claim = dict(LIVE_PAYLOAD["claim"], artifact_ref="sha256:" + "0" * 64)
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), {"ok": True, "claim": claim}, 1.0)

    def test_claim_digest_must_match_current_governed_artifact(self):
        claim = dict(LIVE_PAYLOAD["claim"], artifact_sha256="0" * 64)
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), {"ok": True, "claim": claim}, 1.0)

    def test_claim_effect_must_match_current_governed_effect(self):
        claim = dict(LIVE_PAYLOAD["claim"], requested_effect="different.effect")
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), {"ok": True, "claim": claim}, 1.0)

    def test_claim_principal_must_match_current_principal(self):
        claim = dict(LIVE_PAYLOAD["claim"], principal="agent-a")
        with self.assertRaises(RuntimeError):
            tenki_swarm._normalize_worker_response(plan(), {"ok": True, "claim": claim}, 1.0)

    def test_live_normalization_preserves_exact_payload(self):
        result = tenki_swarm._normalize_worker_response(plan(), LIVE_PAYLOAD, 19.0)
        self.assertEqual(result["status"], "LIVE")
        self.assertTrue(result["live"])
        self.assertFalse(result["authority"])
        self.assertEqual(result["claim"], LIVE_PAYLOAD["claim"])
        self.assertEqual(result["raw"], LIVE_PAYLOAD)
        self.assertEqual(result["source"], "live_http")


if __name__ == "__main__":
    unittest.main()
